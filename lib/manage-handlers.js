...
module.exports = (opts)=>{
  ...
  const buildImage = (docker, name, dockerfile)=>{
    ...
    return new Promise((resolve, reject)=>{
      ...
      docker.buildImage(tar, {t: `${name}`, rm: true, nocache: true}, (err, resp)=>{
      ...
      const containers = yield new Promise((resolve, reject)=>{
        docker.listContainers((err, containers)=>{
          ...
          return resolve(containers.filter((container)=>{
            return container.Image.indexOf("tintoy_") === 0;
          }));
        });
      });
  ...
  handlers.put("/toys/:toyId", (req, res, next)=>{
    ...
    co(function*(){
      ...
      const image = yield buildImage(docker, `tintoy_${name}`, dockerfile);
      ...
      const container = yield runContainer(docker, Object.assign(option, defaultOptions));
      return {image, container, name, dockerfile};
    }).then((toy)=>{
      res.json(toy);
    }).catch(next);
